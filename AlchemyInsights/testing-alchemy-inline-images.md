---
title: Ispitivanje alhemije inline slike
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: 0b06093dabd685f76cc39b1d7c0f8b03883b6e79
ms.translationtype: MT
ms.contentlocale: hr-HR
ms.lasthandoff: 10/25/2019
ms.locfileid: "36743121"
---
# <a name="access-the-domains-page"></a><span data-ttu-id="42bb8-102">Pristup stranici domena</span><span class="sxs-lookup"><span data-stu-id="42bb8-102">Access the Domains Page</span></span>

<span data-ttu-id="42bb8-103">Da biste pristupili stranici domena, samo kliknite ovu ikonu u nastavku:</span><span class="sxs-lookup"><span data-stu-id="42bb8-103">To access the domains page, just click this icon below:</span></span>

  
<img src='data:image/png;base64,'/>

<span data-ttu-id="42bb8-104">Zatim možete dodati, izmijeniti ili ukloniti domenu.</span><span class="sxs-lookup"><span data-stu-id="42bb8-104">Then you can add, modify, or remove your domain.</span></span>
<span data-ttu-id="42bb8-105">Dodatne informacije potražite u odjeljku [Najčešća pitanja o domenama.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span><span class="sxs-lookup"><span data-stu-id="42bb8-105">For more information, see the [Domains FAQ.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span></span>
  
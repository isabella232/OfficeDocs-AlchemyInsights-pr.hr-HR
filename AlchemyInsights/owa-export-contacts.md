---
title: '8000020: izvoz kontakata'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: 311964b1e8e049ad7c26b106d5fe551f2d348170
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: hr-HR
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694195"
---
# <a name="export-contacts"></a>Izvoz kontakata

1. Pri dnu stranice u programu Outlook na webu odaberite <img src='data:image/png;base64,' />
 **Osobe**.
2. Na alatnoj traci odaberite **Upravljanje**  >  **uvoznim kontaktima**. 
3. Odaberite da biste izvezli sve kontakte ili samo kontakte iz određene mape, a zatim odaberite **Izvoz**.
4. Pri dnu stranice odaberite **Spremi** da biste spremili "contacts.csv" u zadanu mapu za preuzimanje. Ili odaberite strelicu uz stavku **Spremi** i odaberite **Spremi kao**.

Dodatne informacije potražite u članku [izvoz kontakata iz programa Outlook](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

